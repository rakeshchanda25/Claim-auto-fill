/* Shared by the tab handlers below, which are otherwise in separate scopes. */
...
(function initAITab() {
    ...
    const jurisdictionSelect = document.getElementById('ai-jurisdiction');
    ...
    function refreshJurisdiction() {
        ...
        jurisdictionGroup.style.display = '';
    }
    ...
    generateBtn.addEventListener('click', async () => {
        ...
        if (jurisdictionSelect.value) fd.append('jurisdiction', jurisdictionSelect.value);
        if (seedInput.value) fd.append('seed', seedInput.value);
        if (userInputEl.value.trim()) fd.append('user_input', userInputEl.value.trim());
