...
from .state_forms import state_form
...
def render_html(template_name: str, data: dict) -> str:
    ...
    layout = get_layout(doc_type, key, data.get("layout_shape"))
    if layout is not None:
        return render_template_string(layout, data)
    meta = data.get("state_form") or state_form(key)
    if meta:
        archetype = _env.get_template(f"_archetypes/{meta['archetype']}.html")
        return archetype.render(data=data, **{**data, "state_form": meta})
