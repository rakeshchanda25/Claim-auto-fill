...
from renderers.layouts import available_layouts, save_layout
...
from .registry import PACKET_REGISTRY, layout_axis
...
def begin_run(reference_bytes: bytes | None = None, custom_fields: dict | None = None,
              anchor_date: str | None = None, jurisdiction: str | None = None) -> None:
    global _ctx
    _ctx = RunContext(
...
def _stage_doc_data(data: dict, doc_type: str, scenario: str) -> dict:
    ...
    key = data.get("layout_key")
    if key:
        summary["jurisdiction"] = key
        summary["layout_available"] = key in available_layouts(resolve_doc_type(doc_type))
    return summary
...
def generate_synthetic_data(doc_type: str, scenario: str = "general", seed: int = None,
    ...
    data = build_synthetic_data(doc_type, scenario,
                                ...
                                jurisdiction=current_run().jurisdiction)
...
def author_layout(doc_type: str, jurisdiction: str, html: str) -> dict:
    """Save a jurisdiction-specific Jinja layout for a document type.

    Use this ONLY when generate_synthetic_data reports layout_available=false for
    the requested jurisdiction. Write a complete standalone HTML document that
    reproduces that jurisdiction's real official form - its form number, agency
    name, section order and field labels - using Jinja placeholders for the data
    (e.g. {{ incident_date }}, {{ parties_involved[0].name }}). Read the state's
    spec with read_skill_file first; do not invent field names.

    The layout must render the document's required fields and must keep the
    SPECIMEN watermark and the synthetic-data footer - it is rejected otherwise.

    Saved once and reused for every later document in that jurisdiction. Returns
    whether it was accepted, and every problem found if it was not.
    """
    resolved = resolve_doc_type(doc_type)
    if layout_axis(resolved) != "state":
        raise ValueError(
            f"{resolved} is a fixed national form - it must not have per-jurisdiction "
            f"layouts. Render it with the standard template."
        )

    data = build_synthetic_data(resolved, "general", jurisdiction=jurisdiction)
    problems = validate_layout(html, data, _REQUIRED_FIELDS.get(resolved, ()))
    if problems:
        return {"status": "rejected", "jurisdiction": jurisdiction, "problems": problems}

    save_layout(resolved, jurisdiction, html)
    return {
        "status": "saved",
        "doc_type": resolved,
        "jurisdiction": jurisdiction,
        "available": available_layouts(resolved),
    }
