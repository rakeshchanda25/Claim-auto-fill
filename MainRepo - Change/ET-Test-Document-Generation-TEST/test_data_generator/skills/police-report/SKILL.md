...
allowed-tools: generate_synthetic_data render_document_to_pdf validate_document_structure author_layout
---
...
## Layout variants by state

Every US state publishes its own crash report form, so this document type varies by jurisdiction
as well as by scenario.

When `generate_synthetic_data` returns `jurisdiction` with `layout_available: false`, that state
has no layout yet. To create one:

1. `read_skill_file('police-report', 'references/states/<CODE>.md')` for the sourced field list.
   If that file does not exist, or its fidelity is `unsourced`, **do not invent field names** -
   render with the generic template instead.
2. Author a complete HTML document reproducing that state's real form, using Jinja placeholders
   for the data, honouring the design invariants in `references/rendering-spec.md`.
3. Call `author_layout(doc_type, jurisdiction, html)`. It validates and caches on success, or
   returns the problems to fix.

The layout is saved once and reused for every later document in that state, so this happens only
on the first request for a state.

Layouts must handle both scenario shapes - check `components` before rendering vehicle sections.
