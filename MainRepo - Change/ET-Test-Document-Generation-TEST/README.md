...
test data. A test enforces this.
...
│   └── config.py
├── sourcing/state_forms.yaml              # provenance for every per-state layout
├── renderers/
│   ├── layouts/<doc-type>/<STATE>.html    # per-state forms (TX = CR-3, CA = CHP 555)
│   ├── layouts.py                         # layout cache
│   ├── layout_validator.py                # gate a layout must pass to be cached
...
342 checks. Every document type renders through its real template with
